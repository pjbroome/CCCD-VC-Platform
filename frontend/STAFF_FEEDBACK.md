...
- **Conversion-ux audit round** (Patrick: "recommit to using that info" — 6-dimension agent audit of the page against MASTER_GUIDE/CHECKLISTS, 36 findings → 12 taste-gated → 9 applied):
  ...
  - **Sutton example photos in the upload boxes** (Patrick 2026-07-08): line-art sketches replaced with anchor-locked Sutton renders — 4:5 golden-hour selfie in the face box, 5:4 smile close-up in the smile box — gold viewfinder brackets overlaid, small "Example" chip so nobody reads it as an uploaded photo, label now "Tap to add yours". 4K masters archived in Sutton_LOOK_LIBRARY; web copies ~250KB/180KB
  - **Staff photo viewer BUILT (Patrick approved backend build 2026-07-07):** PhotoEditor upgraded to a full viewer — wheel/pinch/slider/double-click zoom to 500% with drag-to-pan, rotate (persists via "Save orientation" where editable), **Download** (full-res copy with rotation baked in), ← → browsing across a patient's photos, Esc/arrows trapped so an open viewer never advances presentation slides. Wired to: dashboard row thumbnails (click), detail-page photos (click), and the presenter patient slide (click, read-only). Component mechanics verified live in a local harness (zoom %, rotate transform, per-photo view reset, position label, Esc)
  - Presenter patient slide no longer crops the patient's photos: contain + blurred self-backdrop (was `object-cover` — cropping the smile mid-consult)
  - Viewer controls compacted (Patrick 2026-07-07): slider + text labels + helper line removed → four small icon bubbles (rotate ↺ ↻ · zoom − +) bottom-right under the photo; wheel/pinch/drag/double-click zoom still work, hit areas quietly extended past the 32px glyphs
  - Viewer opens full-size (Patrick 2026-07-07): modal is now h-[92vh] max-w-6xl, photo stage fills ~75% of the viewport (was a 640px box). Gotcha fixed: `flex-1` (flex-basis 0%) overrode a fixed stage height on the column main axis — gave the modal a definite height so flex-1 fills it instead
  - Dashboard: face thumbnail enlarged 36px → 48px (still compact, now big enough to recognize the patient); **entire row is clickable** → opens the patient submission (status dropdown, photo thumb, and View button all stopPropagation so they keep their own behavior)
  - Patient detail info now shows **City and Zip as separate rows** (parsed from the single combined location field "28202 — Charlotte, NC"; falls back cleanly when only a raw zip was submitted)
